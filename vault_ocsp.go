...
func (source VaultSource) Response(request *ocsp.Request) ([]byte, http.Header, error) {
	...
	if bytes.Compare(request.IssuerKeyHash, caHash) != 0 {
		return nil, nil, fmt.Errorf("Request issuer key has does not match CA subject key hash")
	}
	...
	case json.Number:
		...
		if err != nil {
			return nil, nil, fmt.Errorf("Could not convert revocation time to int64 value")
		}
		...
		if block == nil {
			return nil, nil, fmt.Errorf("could not decode PEM data")
		}
